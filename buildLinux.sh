...
make ARCH=arm64 CROSS_COMPILE=aarch64-unknown-linux-gnu- clean || { echo "Failed to clean"; exit 1; }
...
yes "" | make ARCH=arm64 CROSS_COMPILE=aarch64-unknown-linux-gnu- oldconfig || { echo "Failed to update .config file"; exit 1; }

# prepare the source tree for compilation
# Prepare the source tree for compilation and check if operation was successful
make ARCH=arm64 CROSS_COMPILE=aarch64-unknown-linux-gnu- prepare || { echo "Failed to prepare source tree for compilation"; exit 1; }

# compile the kernel using all available CPU cores
# Compile the kernel and check if operation was successful
#make -j$(nproc) ARCH=arm64 CROSS_COMPILE=aarch64-unknown-linux-gnu- || { echo "Failed to compile the kernel"; exit 1; }
...
echo "make -jX ARCH=arm64 CROSS_COMPILE=aarch64-unknown-linux-gnu-"
